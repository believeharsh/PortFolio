// import React from 'react'
import ExpHeader from './ExpHeader/ExpHeader';
import CommonExpeDiv from './CommonExpeDiv/CommonExpeDiv';

function Experience() {
  return (
   <>
   <div className="flex flex-col mx-4 pb-5 ">
     
    <div className="">
    <ExpHeader/>

    </div>

    <div className="mx-auto mt-4">
    <CommonExpeDiv ImageUrl="https://officesnapshots.com/wp-content/uploads/2019/03/microsoft-offices-hyderabad-4.jpg"
      logo="data:image/png;base64,"

      company="Microsoft"
      tanure='2 Years and 10 months '
      location="Noida, Uttar Pradesh India"
      skill="Software Developoment"
    />
  
    <CommonExpeDiv/>
    <CommonExpeDiv/>
   


    </div>
 
   </div>

   </>
  )
}

export default Experience;
